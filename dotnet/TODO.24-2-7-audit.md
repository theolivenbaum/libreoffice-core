...
| `Paperless.WordProcessing` | `Ooxml/DocxLayoutSource.cs` :777 (`w:pPr/w:rPr` is the paragraph *mark's* formatting) | 63 | **verified 26.2.4.2, 2026-08-21** — all three claims, on the reference alone, each with a control: a `Heavy` paragraph whose mark says `<w:b w:val="0"/>` is drawn **Bold** while the same style with the *run* saying it is drawn regular; a mark saying `<w:b/><w:sz w:val="48"/>` leaves the text 10.00 pt upright where the run gives 24.00 pt Bold; and an empty paragraph whose mark states 36 pt has a baseline pitch of 52.90 against a control's 23.00, which is 36 x 1.15 exactly. `probes/words-r63/audit_markstyle.py`. **The last open site under `Paperless.WordProcessing` that a corpus document can reach** — see below |
...
| 63 (words) | `Ooxml/DocxLayoutSource.cs` :777, three claims in one site | **all three verified** — and the useful half of this entry is the *classification* beside it. `Paperless.WordProcessing`'s eleven open hits are not eleven open sites: five are prose inside an existing marker (`WordCompatibility.cs`, `WriterPoolSpacing.cs` x2, `WordStyles.cs` :316, and the `Paginator.cs` note that *supersedes* a 24.2.7.2 claim), and five more state a 26.2.4.2 measurement in the same comment as the 24.2.7.2 one they replaced — two of those in the ODT and RTF readers, whose formats have **no witness in this corpus** (271 `.docx`, 66 `.doc`, 0 `.rtf`, 0 `.odt`). That left exactly one, and it is now closed. **The header's warning that the string over-reads is quantified here for the first time: 11 hits, 1 reachable open site.** `probes/words-r63/audit_markstyle.py` |
| 58 (words) | `Paperless.Text/Layout/LineBreaker.cs`, the number-opening hyphen | **WRONG, and fixed.** The site's own five worked examples were the evidence and three of them are false on 26.2.4.2 — `E-22`, `$-22` and `A -222` do **not** break after the hyphen, and neither do `abc-22` or a hyphen opening the text; only `10-19` and `5-2` do. One rule covers all ten authored cases: **a hyphen opens a number unless a digit precedes it.** The method needed no width tuning — each token is longer than its line in a six-character column, so *where* it breaks separates the rules outright, and two controls with known answers ran with it (`abcd-efghij` breaks after its hyphen on both sides; `(222` does not break after the bracket). `probes/words-r58/audit_hyphenbreak.py`. **The lesson is the one this file already carries and it arrived from the other side**: the old comment's worked examples were what made it checkable, and they were what refuted it. A site that shows its measurements is worth more than one that states a conclusion, even when the measurements are wrong |
...
**The open count *does* fall when a site is FIXED and its old prose was wrong.** Round 58's
`LineBreaker.cs` site did not merely have a stale figure — its four worked examples were false, so
they were replaced by the ones measured on 26.2.4.2 rather than kept as a record of what the code
was fitted to. The commands give **38 open** at that commit against 39 before it, and
`Paperless.Text` 5 against 6. There is nothing to preserve in a sentence that is wrong about the
binary it names.
...
|---|---|---|---|
| 2026-08-21 | `Paperless.WordProcessing/Ooxml/WordStyles.cs` `HasDefaultParagraphPropertiesElement` (a bare `w:pPrDefault` turns widow/orphan control on) | **VERIFIED** | round 62, `probes/words-r46/widow-orphan-default.py` re-run unchanged on 26.2.4.2 — 9 variants x 5 straddle positions, 45 renderings. The discriminating pair is `no-pPrDefault` against `empty-pPrDefault`: at 14 and 16 fillers the first puts 3 and 1 target lines on page one, the same as the `para-off` room control, and the second puts 2 and 0, the same as `para-on`. Presence with no `w:pPr` inside it is what does it. Three arms say default and not override — a `w:pPrDefault` *stating* `w:widowControl w:val="0"` is off, a paragraph stating it is off, `w:docDefaults` removed entirely is off. And `settings-on` is **inert at every filler count**, reconfirming HANDOVER §7's refutation of the document-level `w:settings/w:widowControl` on the current binary |
...
The running score across all rounds is now **three wrong in sixteen**. Two of the three were shared-layer sites; the third is `SheetPageDecoration.cs`. **Ten** `Paperless.Spreadsheets` sites have been re-checked and nine were correct.
