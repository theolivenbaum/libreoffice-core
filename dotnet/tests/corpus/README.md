...
| `slide-text-area-grid.pptx` | That a shape's text rectangle is an integer number of hundredths of a millimetre, which is the unit `SdrObject`'s `tools::Rectangle` holds and not the EMU the file states. Three boxes with all four insets at zero and one 18 pt run each, whose left and top edges are 2540.497, 5080.503 and 8890.497 units — two directions of rounding on two axes, so a constant shift cannot pass. The whole effect is half a unit, 0.0142 pt, which is why the assertions carry a 0.005 pt tolerance rather than this suite's usual 0.1: at 0.1 the test could not fail. LibreOffice draws 72.000 and 144.028 pt across and a 216.000 pt baseline separation; unquantised gives 72.0141, 144.0143 and 216.0141 |
| `slide-autofit-paragraph-space.pptx` | Whether the shrink-to-fit search's line-spacing scale also reaches a paragraph's own `a:spcBef`, which no corpus deck can settle: the two readings differ only where the search's answer tips between a larger font at reduced spacing and a smaller one at full spacing, and a corpus page carries a dozen other differences at once. Three boxes of one text with all four insets at zero, four paragraphs of two lines each with the second line forced by an `a:br` so the line count cannot move with the font size, and a 12 pt `a:spcBef` on every paragraph; a throwaway shape goes first on the slide because LibreOffice's shared draw outliner formats the first text object on a page before `SetFixedCellHeight` takes hold. The heights are the discriminating three out of a twelve-box sweep: at 150 pt LibreOffice draws 11.99 pt where scaling only the lines gives 15.00 at four-fifths, at 200 pt it draws 17.01 where scaling only the lines gives 18.99 at nine-tenths, and 210 pt is a control both readings put at 18.00 so a blanket shift downwards cannot pass |
| `slide-picture-washout.pptx` | What a blip's `a:lum` does to a picture, which is not what it says. One generated 0–255 ramp — grey, then two colour sweeps, so every level of every channel appears — drawn four times: untouched, at `bright="70000" contrast="-70000"`, at `bright="40000"` alone, and at `bright="20000" contrast="20000"`. Those are the control and the three separate arithmetics `GraphicProperties::pushToPropMap` sorts the pair into: PowerPoint's washout is mapped to LibreOffice's watermark mode and applied as **+50 and −70**, both-stated is baked with MSO's own formula, and one alone goes through the colour modifier. The image is generated rather than borrowed so the file carries no licence question |
