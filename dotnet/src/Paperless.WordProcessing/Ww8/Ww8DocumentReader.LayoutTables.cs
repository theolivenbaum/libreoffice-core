...
/// <param name="Restart">Where the count begins again, from the DOP's <c>rncFootnote</c> or <c>rncEdn</c>.</param>
...
{
    /// <summary>
    /// True for a picture set in the run of text rather than floating beside it.
    /// </summary>
    /// <remarks>
    /// The two arrive by completely different routes and meet only here. A floating shape is an
    /// <c>FSPA</c> in the anchor table naming a shape in the drawing; an inline one has no <c>FSPA</c>
    /// at all — its run carries a <c>sprmCPicLocation</c> into the picture stream, and the
    /// <c>PICF</c> there is followed by a lone <c>SpContainer</c>. So the anchor of an inline frame is
    /// synthesised from the <c>PICF</c>'s own goal size, and nothing in it came from the anchor table.
    /// </remarks>
