...
/// LibreOffice bitmap.</strong> <c>drawIconSets</c> (<c>sc/source/ui/view/output.cxx</c>:955-985)
/// paints <c>ScIconSetFormat::getBitmap</c>'s image at ten points square in the cell's
/// bottom-left corner, and that image is one of the application's own icon-theme assets rather
/// than anything derivable from the file. Every other part of the family is readable and is
/// written down in <c>probes/cond-format-r97/results.md</c>: which bucket a value falls into
/// (<c>GetIconSetInfo</c>, <c>colorscale.cxx</c>:1185-1252 — the *last* entry whose threshold the
...
/// document that opens as an OPC spreadsheet: <strong>20 rules in 9 documents</strong> — 2 in the
/// main namespace and 18 stated only in the <c>x14</c> extension list — and 26.2.4.2's own
/// renderings of those nine draw <strong>60 icons over 1549 pt²</strong> in seven of them.
/// </para>
/// </remarks>
