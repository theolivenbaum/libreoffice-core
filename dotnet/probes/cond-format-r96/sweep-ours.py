...
# Which renderings to keep on disk. The hash is what a confinement check needs and it is written
# for every document either way; the PDF itself is only needed for the handful a round then scores
# on ink. `/` filled to 94% during this round with three sweeps running, and on this filesystem a
# write fails while a delete succeeds — which leaves truncated PDFs that score as real differences.
# So name a keep list, one identity per line, and everything else is hashed and deleted.
KEEP = (set(pathlib.Path(os.environ["SWEEP_KEEP"]).read_text(encoding="utf-8").splitlines())
        if os.environ.get("SWEEP_KEEP") else None)
...
def render(args):
    ...
    if KEEP is None or ident in KEEP:
