...
for v in (2000,5000,9000):
    ...
    open(f'in_at{v}.txt','w').write('\n'.join(lines)+'\n')
    if not os.path.exists(f'ours_at{v}.tsv'):
        with open(f'ours_at{v}.tsv','w') as fh:
            subprocess.run(CLI+[os.path.abspath(f'in_at{v}.txt'),str(W),str(H)],stdout=fh,check=True)

refs={v: parse_svg(f'out/vtall{v}.svg',W) for v in (2000,5000,9000)}
ours={v: [l.rstrip('\n').split('\t') for l in open(f'ours_at{v}.tsv')] for v in (2000,5000,9000)}
