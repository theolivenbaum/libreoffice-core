...
if __name__ == "__main__":
    ...
    for path in sorted(glob.glob(os.path.join(HERE, "fixtures", "*.docx"))):
        name = os.path.basename(path)[:-5]
        for label, prefix in (("obstacle", "QUICK LINK 1"),
                              ("heading", "INFORMATION"),
                              ("body", "PROSE line 1"),
                              ("index", "INDEX ENTRY 0")):
            r = first(os.path.join(HERE, "ref", name + ".pdf"), prefix)
            o = first(os.path.join(HERE, "ours", name + ".pdf"), prefix)
