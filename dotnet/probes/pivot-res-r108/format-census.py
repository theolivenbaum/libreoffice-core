#!/usr/bin/env python3
"""What the pivot parts' own `<format>` records ask for, and whether the reference honours it.
...
tree reads none of them. The census is per pivot part: how many `<format>` elements it states,
...
def main():
    for path in sys.argv[1:]:
        ...
        for part in sorted(n for n in z.namelist()
            ...
            formats = list(root.findall(m('format')))
            if not formats:
                continue
            total += len(formats)
