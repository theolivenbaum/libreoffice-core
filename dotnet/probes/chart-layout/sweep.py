"""Re-sweep one or more tracks against the gate's own banked reference PDFs.
...
Generalised from `probes/chart-cat-reverse/slides-sweep.py`, which did the slides track only;
this round touches `Core/Charts` and `Ooxml`, so all three tracks have to be swept.

    sweep.py <out-dir> [--track words|slides|sheets|all] [--validate-only]
"""
import os
import subprocess
import sys
...
CLI = os.environ.get(
    "PAPERLESS_CLI",
    "/home/user/wt-chartlayout/dotnet/tools/Paperless.Cli/bin/Debug/net10.0/linux-x64/"
    "Paperless.Cli")
...
def main():
    ...
    prefixes = ["words/", "slides/", "sheets/"] if track == "all" else [track + "/"]

    want = stored(prefixes)
    paths = sorted(want)
