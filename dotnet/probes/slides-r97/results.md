...
| `bullet-p2.txt` | O25's two numbers off the content stream's own `Td`/`Tm`, beside O15's census page |
...
| O25's two numbers | measured off `Td`/`Tm`, not off a bounding box (§2.1, `bullet-p2.txt`) |
...
- **Page 2 and page 22.** O25's witness bullet and O15's census entry for the same deck are
