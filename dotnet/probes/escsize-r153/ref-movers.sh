...
cd "$(dirname "$0")"
OUT=/home/user/escsize-r153-sweep/ref
mkdir -p "$OUT"
...
rows = []
for folder in (pathlib.Path('/home/user/corpus-odf/rtf'),
               pathlib.Path('/home/user/corpus-odf/odt'),
               pathlib.Path('/home/user/sample-files')):
...
cut -f2 movers-paths.tsv | xargs -P 4 -I{} sh -c '
    ...
    timeout -k 30 900 /opt/libreoffice26.2/program/soffice --headless --norestore \
        -env:UserInstallation="file:///tmp/r153-ref-$key" \
