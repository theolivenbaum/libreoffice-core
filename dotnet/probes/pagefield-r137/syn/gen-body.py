...
def build_body(path, grouped, intable):
    para = '<w:p>' + drawing(grouped) + '</w:p>'
    blk = (TBL_OPEN + para + TBL_CLOSE) if intable else para
    doc = DOCUMENT.replace('<w:body>', '<w:body>' + blk)
    empty_footer = ('<?xml version="1.0" encoding="UTF-8" standalone="yes"?>'
                    '<w:ftr %s><w:p/></w:ftr>' % NS)
    z = zipfile.ZipFile(path, 'w', zipfile.ZIP_DEFLATED)
    z.writestr('[Content_Types].xml', CT); z.writestr('_rels/.rels', RELS)
    z.writestr('word/document.xml', doc)
    z.writestr('word/_rels/document.xml.rels', DOCRELS)
    z.writestr('word/settings.xml', SETTINGS); z.writestr('word/styles.xml', STYLES)
    z.writestr('word/fontTable.xml', FONTTABLE); z.writestr('word/footer1.xml', empty_footer)
    z.writestr('docProps/core.xml', CORE); z.writestr('docProps/app.xml', APP)
    z.close(); print('built', path)

out = sys.argv[1]
build_body(os.path.join(out, 'syn-body-group-para.docx'),  True,  False)
build_body(os.path.join(out, 'syn-body-group-table.docx'), True,  True)
build_body(os.path.join(out, 'syn-body-plain-table.docx'), False, True)
