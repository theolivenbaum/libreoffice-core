...
| O21 | **A cell's drawn text and its compared value are two different strings.** `RichStringPortion::setText` is `maText = AttributeConversion::decodeXString(rText)` and nothing else (`sc/source/filter/oox/richstring.cxx`:60-63), so every control character survives into the document model and the eliding `XlsxCellText.Of` describes is the *drawing* layer's. A conditional format compares the stored spelling: `"\tD5758620001301"` and `"D5758620001301"` are two `duplicateValues` keys, and `containsBlanks`' `LEN(TRIM(#B))=0` reads a lone `U+0009` as **not** blank because `TRIM` takes spaces and not tabs. `XlsxCellText.Stored` and `XlsxSharedStrings.StoredAt` keep the second spelling only where it differs from the first. | `probes/cond-format-r97` §1. Fixture `sheet-cf-stored-vs-drawn.xlsx`, seven cells, every expectation 26.2.4.2's own PDF; fails at the base and passes after. Reach **1 cell in 1 of 947 renderings**: `ST Capability List Rev.16` goes 18 `#9C0006` spans → **16** against the reference's 16, on the reference's own four pages, `\|ink\|%` 26.85 → 26.80. |
| O22 | **`dataBar` is read**, in `XlsxDataBars` beside the colour scale. Four arms are not what 18.3.1.10 would have you write. `minLength`/`maxLength` are used **only** in the `NONE` and `MIDDLE` axis arms (`GetDataBarInfo`, `colorscale.cxx`:1004-1043) and `DataBarRule`'s constructor leaves the axis at `NONE` (`condformatbuffer.cxx`:351-356) — so an `x14:dataBar` with no `axisPosition` turns them off, because anything but `none`/`middle` becomes `AUTOMATIC` (`:1630-1643`). The extension's own `minLength`/`maxLength` are **never read**; `importDataBar` (`:1710-1715`) takes `gradient` and `axisPosition` alone. Its `x14:cfvo` types overwrite the main namespace's, which is the only way `autoMin`/`autoMax` arrive, and a `num`'s `<xm:f>` is taken only when it parses whole (`:1665-1703`). And `mbNeg` is the presence of an `x14:negativeFillColor` and nothing else, so the `COL_LIGHTRED` fallback is unreachable from OOXML. `showValue="0"` takes the cell's value off the page rather than hiding it (`output2.cxx`:1691-1697). | `probes/cond-format-r97` §2. Four fixtures, every expectation 26.2.4.2's own PDF; the geometry reproduces its bars to 0.02 pt on width and 0.12 on position. Reach **9 rules in 6 documents**; see §4 for the corpus figure. The `iconSet` half is **not** closed — it is re-seated as O25 with a corrected census. |
...
| O25 | **`iconSet` is unread, and its reach is four times what r96 recorded.** Not 2 rules in 2 documents but **20 in 9**: 18 of them are stated only in the worksheet's `x14` extension list, with their own `xm:sqref` and no main-namespace `cfRule`, and 26.2.4.2 imports and paints them — `088_To-do_list`'s `H3:H7` set exists nowhere else and the reference draws it. Everything but the glyph is read out of 26.2 in r97 §6: the bucket is the **last** entry whose `Compare` holds (`GetIconSetInfo`, `colorscale.cxx`:1185-1252, no `break`), each entry carries its own mode with `gte="0"` turning `EqGreater` into `Greater`, `mbReverse` reflects the index afterwards, and a custom `NoIcons` entry is stored as index **−1** so `GetIconSetInfo` answers `nullptr` and the cell keeps its own text whatever `showValue` says. **What blocks it is artwork**: `drawIconSets` (`output.cxx`:955-985) paints `ScIconSetFormat::getBitmap`'s image, one of LibreOffice's own icon-theme assets, and nothing in the file derives it. Suppressing the value without drawing the glyph is further from the reference than doing nothing. | `probes/cond-format-r97` §3 and §6, `drawrule-census.txt`, `icon-ink.txt`. **Reach 20 rules in 9 documents; 26.2.4.2 draws 60 icons over 1549 pt² in seven of them** and none in the other two |
