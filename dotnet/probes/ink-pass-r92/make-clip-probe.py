#!/usr/bin/env python3
"""Author a minimal .xlsx whose one filled rectangle straddles a page-column and a
page-row break, so the reference can be asked whether it clips a drawing to the
page's own cell block or to the paper.

Usage: make-clip-probe.py OUT.xlsx [--cols N] [--rows N] [--from-col C --to-col C]
"""
...
def main():
    ...
    drawing = (
        ...
        f'<xdr:to><xdr:col>{a.to_col}</xdr:col><xdr:colOff>0</xdr:colOff>'
        ...
        '</xdr:spPr><xdr:style/></xdr:sp><xdr:clientData/></xdr:twoCellAnchor>'
        '</xdr:wsDr>')
